  ...
  return (
    <section className="rounded-lg border border-stone-200 bg-white p-5 shadow-sm">
      <div className="mb-5 flex items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-ink">{t("top10Holdings")}</h2>
          <p className="text-sm text-stone-500">{t("rankedByMarketValue")}</p>
        </div>
      </div>
        ...
        {topHoldings.map((holding) => (
          <div key={holding.cusip ?? holding.issuerName ?? "holding"} className="grid gap-2 md:grid-cols-[220px_1fr_120px] md:items-center">
            <div className="min-w-0">
              ...
              <p className="text-xs text-stone-500">{holding.portfolioWeight.toFixed(2)}% {t("ofPortfolio")}</p>
            </div>
